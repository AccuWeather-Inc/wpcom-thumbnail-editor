...
class WPcom_Thumbnail_Editor {
	...
	/**
	 * Load plugin assets helper.
	 *
	 * @link https://developer.wordpress.org/block-editor/how-to-guides/javascript/js-build-setup/#dependency-management
	 *
	 * @param string $file_name The name of the file without any extension.
	 *
	 * @return array|null The array of build assets generated by wp-scripts.
	 */
	private function load_asset( $file_name = 'index' ) {
		static $asset;

		if ( empty( $asset ) ) {
			$asset_file = __DIR__ . '/build/' . $file_name . '.asset.php';
	...
	/**
	 * Loads the Thumbnail Editor scripts only on the upload or attachment pages.
	 *
	...
	public function enqueue_admin_scriptreset( $hook ) {
		 ...
		 *
		 * @param array $supported_post_types Supported post types starting with 'attachment'
		 */
		$supported_post_types = apply_filters( 'wpcom_thumbnail_editor_supported_post_types', array( 'attachment' ) );
		if (
			'upload.php' === $hook
	...
	/**
	 * Generates the available thumbnails for editing.
	 *
	 * @param array   $sizes      The supported thumbnail sizes.
